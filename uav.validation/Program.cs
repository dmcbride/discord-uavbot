...
class Program
{
    ...
    public async Task ExtractPlayerId(string file)
    {
        var extractor = new ExtractPlayerId();
        var id = await extractor.Extract(file);
        Console.WriteLine(id);
    }

    private Task BaseCredits()
    {
        var c = new Credits();

        foreach (var exp in Enumerable.Range(7, 103))
        {
            var gv = GV.FromNumber(Math.Pow(10, exp));
            var credits = c.TierCredits(gv);

            Console.WriteLine($"{gv} gets {credits}");
        }
